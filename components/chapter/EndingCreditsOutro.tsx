...
type EndingCreditsOutroProps = {
  onComplete: () => void;
};

const GROK_SEARCHER_URL = "https://www.groksearcher.com";

export function EndingCreditsOutro({ onComplete }: EndingCreditsOutroProps) {
  ...
  useEffect(() => {
    const credits = window.setTimeout(() => setCreditsVisible(true), 500);
    const lineOne = window.setTimeout(() => setLineOneVisible(true), 2400);
    const lineTwo = window.setTimeout(() => setLineTwoVisible(true), 3400);
    const link = window.setTimeout(() => setLinkVisible(true), 4400);
    const proceed = window.setTimeout(() => setReady(true), 5200);
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-lg text-center">
        <p
          className={cn(
            "grok-outro-credits font-mono text-[10px] uppercase tracking-[0.42em] text-zinc-500",
            creditsVisible && "grok-outro-line-visible",
          )}
        >
          Made by a Bonga Developer
        </p>

        <div
          className={cn(
            "grok-outro-recommendation mt-14 space-y-3",
            lineOneVisible && "grok-outro-recommendation-visible",
          )}
        >
          <p
            className={cn(
          ...
          <p
            className={cn(
              "grok-outro-line font-display text-base font-light leading-relaxed tracking-[0.04em] text-zinc-400 sm:text-lg",
