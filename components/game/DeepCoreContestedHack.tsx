...
import {
  ...
  getPersonalizedFortInterferenceLine,
  getPersonalizedFortLockLine,
  getPersonalizedFortTakeoverLine,
  getPersonalizedFortWrongLine,
  type DeepCoreHackConfig,
} from "@/lib/deep-core-hack";
...
type DeepCoreContestedHackProps = {
  config: DeepCoreHackConfig;
...
export function DeepCoreContestedHack({
  config,
  context,
  onSuccess,
  onCancel,
  onGroknetLine,
}: DeepCoreContestedHackProps) {
  ...
  const toggleSlot = useCallback(
    (index: number) => {
      ...
      if (lockedSlots.has(index)) {
        onGroknetLine(
          `Slot ${index + 1} locked — bedrock tremors synchronized. …Wait.`,
          true,
        );
        return;
      }
  ...
  const applyInterference = useCallback(() => {
    ...
    playGroknetInterferenceSound();
    onGroknetLine(
      getPersonalizedFortInterferenceLine(context, interferenceBursts + 1),
      true,
    );
    window.setTimeout(() => {
      setStatus("idle");
  ...
  }, [context, interferenceBursts, onGroknetLine, slots.length]);
  ...
  const applyTakeover = useCallback(() => {
    ...
    playMajorHackTakeoverSound();
    onGroknetLine(
      getPersonalizedFortTakeoverLine(context, takeoverBursts + 1),
      true,
    );
  ...
  const applyLock = useCallback(() => {
    ...
    setStatus("lock");
    onGroknetLine(getPersonalizedFortLockLine(context, slot), true);
    if (lockTimeoutRef.current) clearTimeout(lockTimeoutRef.current);
  ...
  const handleSubmit = useCallback(() => {
    ...
    setStatus("error");
    onGroknetLine(getPersonalizedFortWrongLine(context, nextAttempts), true);
    ...
    if (roll < config.takeoverChance) {
      ...
      roll <
      config.takeoverChance + config.lockChance + config.interferenceChance
    ) {
  ...
  const controlFightLevel = Math.min(
    100,
    50 + interferenceBursts * 8 + takeoverBursts * 15 + attempts * 5,
  );
  ...
  return (
    <div className="terminal-backdrop-in fixed inset-0 z-50 flex items-center justify-center bg-background/96 p-4 backdrop-blur-md">
      <div className="terminal-panel-in major-contested-panel w-full max-w-xl rounded-sm border border-rose-900/40 bg-zinc-950 p-5 shadow-2xl shadow-black/80 sm:p-7">
        <p className="font-mono text-[10px] uppercase tracking-[0.35em] text-rose-400/80">
          Deep Core · {config.label}
        </p>
        <h2 className="mt-2 font-display text-lg font-semibold uppercase tracking-[0.08em] text-zinc-100">
          {config.title}
        </h2>
        <p className="mt-1 font-mono text-[9px] uppercase tracking-widest text-rose-500/60">
          {config.subtitle}
        </p>
        <p className="mt-3 text-sm leading-relaxed text-zinc-400">
          Groknet fights at full presence — bedrock tremors sync to every wrong
          transmit. The last lattice before the plug.
        </p>
        ...
        <div className="mt-4 rounded-sm border border-rose-900/30 bg-rose-950/12 px-3 py-2">
          <div className="flex items-center justify-between font-mono text-[9px] uppercase tracking-widest text-rose-300/75">
            <span>Fortification resistance</span>
            <span>{controlFightLevel}%</span>
          </div>
          <div className="mt-2 h-1.5 overflow-hidden rounded-sm bg-zinc-900">
            <div
              className="major-control-bar h-full bg-gradient-to-r from-rose-700/70 to-amber-500/80 transition-all duration-500"
        ...
        <div className="mt-5">
          <p className="mb-3 font-mono text-[9px] uppercase tracking-widest text-zinc-600">
            FORT sequence · {config.target.length} channels
          </p>
        ...
        {status === "error" ? (
          <p className="mt-4 font-mono text-[10px] uppercase tracking-widest text-red-400/80">
            FORT rejected — Groknet escalates fortification fight
          </p>
        ) : null}
        {status === "success" ? (
...
export { DEEP_CORE_HACK_CONFIG } from "@/lib/deep-core-hack";