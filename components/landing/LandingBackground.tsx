...
import { getLandingParticleCount } from "@/lib/performance-mode";
import { cn } from "@/lib/utils";

export function LandingBackground() {
  const { performanceMode } = usePerformanceMode();
  const particleCount = getLandingParticleCount();
  ...
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden>
      <div
        className={cn(
          "absolute inset-0",
          !performanceMode && "landing-bg-drift",
        )}
      >
        <svg
        ...
        >
          ...
          <ellipse
            ...
            rx="280"
            ry="120"
            fill="url(#distant-glow)"
            className={performanceMode ? undefined : "landing-horizon-pulse"}
          />
            ...
            return (
              <g key={i} opacity={0.35 - i * 0.05}>
                <rect
                  ...
                  height={3}
                  fill="#f97316"
                  className={performanceMode ? undefined : "landing-light-flicker"}
                  style={performanceMode ? undefined : { animationDelay: `${i * 0.7}s` }}
                />
                <rect
                  x={x + width * 0.15}
                  y={y + 18}
          ...
          <rect
            ...
            fill="#f97316"
            opacity="0.15"
            className={performanceMode ? undefined : "landing-door-glow"}
          />
      ...
      {performanceMode ? null : <div className="landing-scan-sweep absolute inset-0" />}
